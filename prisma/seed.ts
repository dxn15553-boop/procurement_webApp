...
async function main() {
  ...
  const teamMember3 = await prisma.user.upsert({
    where: { email: "john@procurex.com" },
    update: {},
    create: {
      name: "John Doe",
      email: "john@procurex.com",
      passwordHash: teamHash,
      role: "TEAM",
      departmentId: departments[0].id,
    },
  ...
  const teamMember4 = await prisma.user.upsert({
    where: { email: "sarah@procurex.com" },
    update: {},
    create: {
      name: "Sarah Smith",
      email: "sarah@procurex.com",
      passwordHash: teamHash,
      role: "TEAM",
  ...
  console.log(`✅ Created users: ${manager.email}, ${teamMember.email}, ${teamMember2.email}, ${teamMember3.email}, ${teamMember4.email}`);
  ...
  const sampleRequests = [
    ...
    },
    {
      ...
      sourceDescription: "Office Furniture for new Operations wing - 50 chairs, 20 desks",
      departmentId: departments[1].id,
    ...
    },
    {
      ...
      nameOfHandler: "Sam Team",
      currentStage: "CS" as const,
      pendingFrom: daysAgo(5),
      pendingDays: 5,
      noOfDays: 5,
      slaStatus: "ON_TRACK" as const,
      createdById: teamMember.id,
      csStatus: "PENDING" as const,
      prStatus: "PENDING" as const,
    },
  ];
